...
def accuracy(y_pred, y_true, nb_old, increment, acc_table, cur_task):
    ...
    for class_id in range(0, np.max(y_true), increment):
        ...
        score = np.around((y_pred[idxes] == y_true[idxes]).sum()*100 / len(idxes), decimals=2)
        all_acc[label] = score
        acc_table[cur_task][index] = score
        index = index + 1
